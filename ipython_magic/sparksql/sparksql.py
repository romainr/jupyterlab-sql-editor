...
class SparkSql(Base):
    ...
    @argument('-j', '--jinja', action='store_true', help='')
    def sparksql(self, line=None, cell=None, local_ns=None):
        ...
        if self.args.refresh.lower() == 'all':
            update_database_schema(spark, output_file, catalog_array)
            update_local_database(spark, output_file)
            return
        elif self.args.refresh.lower() == 'local':
            update_local_database(spark, output_file)
        elif self.args.refresh.lower() != 'none':
            print(f'Invalid refresh option given {self.args.refresh}. Valid refresh options are [all|local|none]')

        sql = self.get_sql_statement(cell, self.args.sql)
        if not sql:
            return
        elif self.args.output.lower() == 'sql':
        ...
        if self.args.view:
            ...
            df.createOrReplaceTempView(self.args.view)
            update_local_database(spark, output_file)
        if self.args.dataframe:
            print(f'Captured dataframe to local variable `{self.args.dataframe}`')
            self.shell.user_ns.update({self.args.dataframe: df})

        limit = self.args.limit
        if limit is None:
            limit = self.limit

        if limit <= 0 or self.args.output.lower() == 'skip' or self.args.output.lower() == 'none':
            print('Query execution skipped')
            return
        elif self.args.output.lower() == 'grid':
        ...
        elif self.args.output.lower() == 'json':
        ...
        elif self.args.output.lower() == 'html':
            header, contents = self.get_results(df, limit)
            if len(contents) > limit:
                print(f'Only showing top {limit} row(s)')
            html = self.make_tag('tr', False,
                        ''.join(map(lambda x: self.make_tag('td', self.args.show_nonprinting, x, style='font-weight: bold'), header)),
                        style='border-bottom: 1px solid')
            for index, row in enumerate(contents[:limit]):
                html += self.make_tag('tr', False, ''.join(map(lambda x: self.make_tag('td', self.args.show_nonprinting, x),row)))
            return HTML(self.make_tag('table', False, html))
        elif self.args.output.lower() == 'text':
            df.show(n=limit, truncate=False)
            return
        else:
            print(f'Invalid output option {self.args.output}. The valid options are [sql|json|html|grid|none].')
